...
class ClangTidyConverter:
    ...
    def __init__(self, basename, outputfolder):
        self.basename = basename
        self.outputfolder = outputfolder

    def print_junit_file(self):
        
        # use list(set()) to provide a list of unique errors (i.e. filter out duplicates)
        sorted_errors = sorted(list(set(self.errors)), key=lambda x: x.file)
        ...
        for file, errorIterator in itertools.groupby(sorted_errors, key=lambda x: x.file):
            ...
            with open(os.path.join(self.outputfolder, os.path.basename(file) + ".clang-tidy.junit.xml"), 'w') as output_file:
                # Write the header.
                output_file.write("""<?xml version="1.0" encoding="UTF-8" ?>
<testsuites id="1" name="{file}" tests="{error_count}" errors="{error_count}" failures="0" time="0">""".format(file=file, error_count=error_count))    
    ...
    def convert(self, input_file):
        ...
        self.print_junit_file()

        # If there are errors, return with error
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        logging.error("Usage: %s base-filename-path output-folder", sys.argv[0])
        logging.error(
            "  base-filename-path: Removed from the filenames to make nicer paths.")
        logging.error(
            "  output-folder: Folder to create the resulting junit xml files in")
        sys.exit(1)
    converter = ClangTidyConverter(sys.argv[1], sys.argv[2])
    converter.convert(sys.stdin)
