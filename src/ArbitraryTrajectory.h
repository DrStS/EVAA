...
public:
     ...
     * \brief Constructor
     */
    ArbitraryTrajectory(size_t numIterations, T delta_t, T amplitude_right, T amplitude_left, T period_right, T period_left, T shift_right, T shift_left) : _numIterations(numIterations), _delta_t(delta_t) {
        if (period_right == 0) {
            _frequencyRight = 0;
        }
        else {
    ...
    * This means, that between two XML points  [X(i), Y(i)] and [X(�+1),
     ...
     */
    void calculateAccelerationsLegs(T& l_long_fl, T& l_long_fr, T& l_long_rl, T& l_long_rr, T& l_lat_fl, T& l_lat_fr, T& l_lat_rl, T& l_lat_rr) {
        ...
        for (int i = 0; i < _numIterations + 1; ++i) {
            ...
            legPointsX_fl[i] = _roadPointsX[i] + localXcoordinates[0] * c - localYcoordinates[0] * s;
            legPointsY_fl[i] = _roadPointsY[i] + localXcoordinates[0] * s + localYcoordinates[0] * c;

            legPointsX_fr[i] = _roadPointsX[i] + localXcoordinates[1] * c - localYcoordinates[1] * s;
            legPointsY_fr[i] = _roadPointsY[i] + localXcoordinates[1] * s + localYcoordinates[1] * c;

            legPointsX_rl[i] = _roadPointsX[i] + localXcoordinates[2] * c - localYcoordinates[2] * s;
            legPointsY_rl[i] = _roadPointsY[i] + localXcoordinates[2] * s + localYcoordinates[2] * c;

            legPointsX_rr[i] = _roadPointsX[i] + localXcoordinates[3] * c - localYcoordinates[3] * s;
            legPointsY_rr[i] = _roadPointsY[i] + localXcoordinates[3] * s + localYcoordinates[3] * c;
        }
     ...
     * \param time at which this happens
     */
    T getVerticalRoadAccelerationsRearRight(size_t iteration) { return _tyreAccelerationsZ_rr[iteration]; }

    /**
     * \brief calculates the position of the t<ew
     * \param time at which this happens
     */
    T getVerticalPositionFrontLeft(size_t iteration) { return _amplitudeLeft * std::sin(_frequencyLeft * (_normedDistance[i] + _phaseShift_fl)); }

    /**
     * \brief calculates the position of the t<ew
     * \param time at which this happens
     */
    T getVerticalPositionFrontRight(size_t iteration) { return _amplitudeRight * std::sin(_frequencyLeft * (_normedDistance[i] + _phaseShift_fr)); }
...
private:
    ...
    T* _tyreAccelerationsX_fl;  // Acceleration that acts on the tyre in X
                                // direction
    T* _tyreAccelerationsX_fr;  // Acceleration that acts on the tyre in X
                                // direction
    T* _tyreAccelerationsX_rl;  // Acceleration that acts on the tyre in X
                                // direction
    T* _tyreAccelerationsX_rr;  // Acceleration that acts on the tyre in X
                                // direction

    T* _tyreAccelerationsY_fl;  // Acceleration that acts on the tyre in Y
                                // direction
    T* _tyreAccelerationsY_fr;  // Acceleration that acts on the tyre in Y
                                // direction
    T* _tyreAccelerationsY_rl;  // Acceleration that acts on the tyre in Y
                                // direction
    T* _tyreAccelerationsY_rr;  // Acceleration that acts on the tyre in Y
                                // direction
