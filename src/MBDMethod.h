...
private:
    ...
    */
...

    {
        ...
        fashion: Original steps for computation of one component: 1.	qc = qc/norm(qc); 2.
        C_Nc = get_basis(qc); 3.	global_z = C_Nc(:,2); 4.	global_z = -global_z /
        norm(global_z); 5.	global_r_fl = pcc + C_Nc*r_fl; 6.	upper_global_spring__fl =
        upper_length(_fl)*global_z; 7.	lower_global_spring__fl = lower_length(_fl)*global_z; 8.
        pw_fl = global_r_fl + upper_global_spring__fl; 9.	pt_fl = pw_fl +
        lower_global_spring__fl; Modified steps for computation of one component: _fl.	qc =
        qc/norm(qc); 2.	C_Nc = get_basis(qc); 3.	global_z = C_Nc(:,2); 4.	global_z =
        -global_z / norm(global_z); 5.	pw_fl = pcc; 6.	pw_fl = pw_fl + C_Nc*r_fl; 7.	pw_fl =
        pw_fl + upper_length(1)*global_z; 8.	pt_fl = pw_fl 8.	pt_fl	= pt_fl +
        lower_length(_fl)*global_z;
        */
        ...
        // 6.	pw1 = pw1 + C_Nc*r1;
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, 1, C_Nc,
                     Constants::DIM, r_fl_, 1, 1, wheel_coordinate_fl_, 1);

        // 7.	pw1 = pw1 + upper_length(1)*global_z;
        mkl<T>::axpy(Constants::DIM, initial_upper_spring_length_[0], global_z, 1,
        ...
        mkl<T>::axpy(Constants::DIM, initial_lower_spring_length_[0], global_z, 1,
        ...
        mkl<T>::axpy(Constants::DIM, initial_upper_spring_length_[1], global_z, 1,
        ...
        mkl<T>::axpy(Constants::DIM, initial_lower_spring_length_[1], global_z, 1,
        ...
        mkl<T>::axpy(Constants::DIM, initial_upper_spring_length_[2], global_z, 1,
        ...
        mkl<T>::axpy(Constants::DIM, initial_lower_spring_length_[2], global_z, 1,
        ...
        mkl<T>::axpy(Constants::DIM, initial_upper_spring_length_[3], global_z, 1,
        ...
        mkl<T>::axpy(Constants::DIM, initial_lower_spring_length_[3], global_z, 1,
    ...
    *********************************************************************/
    /*
    Updates the velocity of the 4 wheels and tyres as well as the angular velocity, such that the
    car is already in the trajectory of the circle overwrites the velocity in the tyres and wheels
    and the angular velocities only keeps the tangential component of the velocity of the car body
    */
    ...
    */
    void get_circular_road_force(T* Fr, T* v, T& m, T* p)
    {
        ...
        // the force is in the same direction as the position vector TODO: take care of situation
        ...
        inv_radius = 1.0 / mkl<T>::nrm2(Constants::DIM, p, 1);
        ...
        velocity_magnitude_tyre =
            mkl<T>::dot(mkl_DIM, v, mkl_incx, velocity_direction_tyre, mkl_incy);
    ...
    */
                                ...
                                double& inv_norm_r_up, double& norm_r_low, double& inv_norm_r_low)
    {
        ...
        */
        ...
        mkl<T>::copy(Constants::DIM, pcc_, 1, cf_r_up_, 1);
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, 1, cf_C_cN,
                     Constants::DIM, r_, 1, 1, cf_r_up_, 1);
        mkl<T>::axpy(Constants::DIM, -1.0, pw_, 1, cf_r_up_, 1);

        // r_low
        mkl<T>::copy(Constants::DIM, pw_, 1, cf_r_low_, 1);
        mkl<T>::axpy(Constants::DIM, -1.0, pt_, 1, cf_r_low_, 1);

        /* Compute spring lengths and their inverses */
        norm_r_up = mkl<T>::nrm2(Constants::DIM, cf_r_up_, 1);

        inv_norm_r_up = 1.0 / norm_r_up;

        norm_r_low = mkl<T>::nrm2(Constants::DIM, cf_r_low_, 1);
    ...
    void compute_angles()
    {
        ...
        */
        ...
        MathLibrary::get_quaternion<T>(cf_r_up_fl, cf_col_dat, cf_upper_angle_fl,
                                       cf_upper_normal_fl, Constants::DIM);
        MathLibrary::get_quaternion<T>(cf_r_up_fr, cf_col_dat, cf_upper_angle_fr,
                                       cf_upper_normal_fr, Constants::DIM);
        MathLibrary::get_quaternion<T>(cf_r_up_rl, cf_col_dat, cf_upper_angle_rl,
                                       cf_upper_normal_rl, Constants::DIM);
        MathLibrary::get_quaternion<T>(cf_r_up_rr, cf_col_dat, cf_upper_angle_rr,
                                       cf_upper_normal_rr, Constants::DIM);

        MathLibrary::get_quaternion(cf_r_low_fl, cf_r_up_fl, cf_lower_angle_fl, cf_lower_normal_fl,
                                    Constants::DIM);
        MathLibrary::get_quaternion(cf_r_low_fr, cf_r_up_fr, cf_lower_angle_fr, cf_lower_normal_fr,
                                    Constants::DIM);
        MathLibrary::get_quaternion(cf_r_low_rl, cf_r_up_rl, cf_lower_angle_rl, cf_lower_normal_rl,
                                    Constants::DIM);
        MathLibrary::get_quaternion(cf_r_low_rr, cf_r_up_rr, cf_lower_angle_rr, cf_lower_normal_rr,
                                    Constants::DIM);
    }
...
lower_force1 = lower_spring_stiffness(1) * (r_low1) * (1 - lower_spring_length(1) *
inv_norm_r_low1); lower_force2 = lower_spring_stiffness(2) * (r_low2) * (1 - lower_spring_length(2)
* inv_norm_r_low2); lower_force3 = lower_spring_stiffness(3) * (r_low3) * (1 -
lower_spring_length(3) * inv_norm_r_low3); lower_force4 = lower_spring_stiffness(4) * (r_low4) * (1
- lower_spring_length(4) * inv_norm_r_low4);
...
*/
        ...
        scale = this->upper_spring_stiffness[0] *
                (1.0 - this->upper_spring_length[0] * inv_norm_r_up_fl);
        ...
        scale = this->upper_spring_stiffness[1] *
                (1.0 - this->upper_spring_length[1] * inv_norm_r_up_fr);
        ...
        scale = this->upper_spring_stiffness[2] *
                (1.0 - this->upper_spring_length[2] * inv_norm_r_up_rl);
        ...
        scale = this->upper_spring_stiffness[3] *
                (1.0 - this->upper_spring_length[3] * inv_norm_r_up_rr);
        ...
        scale = this->lower_spring_stiffness[0] *
                (1.0 - this->lower_spring_length[0] * inv_norm_r_low_fl);
        ...
        scale = this->lower_spring_stiffness[1] *
                (1.0 - this->lower_spring_length[1] * inv_norm_r_low_fr);
        ...
        scale = this->lower_spring_stiffness[2] *
                (1.0 - this->lower_spring_length[2] * inv_norm_r_low_rl);
        ...
        scale = this->lower_spring_stiffness[3] *
                (1.0 - this->lower_spring_length[3] * inv_norm_r_low_rr);
...
*/
        ...
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, 1.0,
                     this->r_fl_tilda, Constants::DIM, wc_, 1, 0.0, cf_temp, 1);
        mkl<T>::copy(Constants::DIM, vc_, 1, cf_upper_dampf_fl, 1);
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, -1.0, cf_C_cN,
                     Constants::DIM, cf_temp, 1, 1.0, cf_upper_dampf_fl, 1);
        // dot((vc - C_cN' * (r1_tilda * wc)), r_up1)
        // std::cout << mkl<T>::dot(Constants::DIM, upper_dampf1, 1, r_up1, 1) << std::endl;
        // std::cout << MathLibrary::dot_product<T>(upper_dampf1, r_up1, Constants::DIM) <<
        // std::endl;
        ...
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, 1.0,
                     this->r_fr_tilda, Constants::DIM, wc_, 1, 0.0, cf_temp, 1);
        mkl<T>::copy(Constants::DIM, vc_, 1, cf_upper_dampf_fr, 1);
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, -1.0, cf_C_cN,
                     Constants::DIM, cf_temp, 1, 1.0, cf_upper_dampf_fr, 1);
        ...
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, 1.0,
                     this->r_rl_tilda, Constants::DIM, wc_, 1, 0.0, cf_temp, 1);
        mkl<T>::copy(Constants::DIM, vc_, 1, cf_upper_dampf_rl, 1);
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, -1.0, cf_C_cN,
                     Constants::DIM, cf_temp, 1, 1.0, cf_upper_dampf_rl, 1);
        ...
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, 1.0,
                     this->r_rr_tilda, Constants::DIM, wc_, 1, 0.0, cf_temp, 1);
        mkl<T>::copy(Constants::DIM, vc_, 1, cf_upper_dampf_rr, 1);
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, -1.0, cf_C_cN,
                     Constants::DIM, cf_temp, 1, 1.0, cf_upper_dampf_rr, 1);
...
*/
        ...
        mkl<T>::axpy(Constants::DIM, 1.0, FT_fl, 1, cf_local_FR_fl, 1);
        ...
        mkl<T>::axpy(Constants::DIM, 1.0, FT_fr, 1, cf_local_FR_fr, 1);
        ...
        mkl<T>::axpy(Constants::DIM, 1.0, FT_rl, 1, cf_local_FR_rl, 1);
        ...
        mkl<T>::axpy(Constants::DIM, 1.0, FT_rr, 1, cf_local_FR_rr, 1);
    ...
    void compute_car_body_total_torque()
    {
        ...
        springs r2_tilda * (C_cN * sum_car_force2) + ... r3_tilda * (C_cN * sum_car_force3) + ...
                                                        r_rr_tilda * (C_cN * sum_car_force4) + ...
                                                   -C_cN * upper_S1 - C_cN * upper_S2 - C_cN *
        upper_S3 - C_cN * upper_S4 + ...     % ??from the rotational spring -get_tilda(wc) * Hc +
        Tc;                       % from angular momentum and external torques

        */

        // Hc = A(1:3, 1:3) * wc;
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, 1.0, this->Ic,
                     Constants::DIM, wc_, 1, 0.0, cf_Hc, 1);
        MathLibrary::get_tilda<T>(wc_, cf_wc_tilda);
        mkl<T>::copy(Constants::DIM, cf_Hc, 1, cf_temp, 1);
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, -1.0, cf_wc_tilda,
                     Constants::DIM, cf_temp, 1, 0.0, cf_Hc, 1);
        ...
        mkl<T>::gemv(CblasRowMajor, CblasTrans, Constants::DIM, Constants::DIM, -1.0, cf_C_cN,
                     Constants::DIM, cf_upper_S_rr, 1, 1.0, cf_sum_torque_spring_car, 1);
        mkl<T>::gemv(CblasRowMajor, CblasTrans, Constants::DIM, Constants::DIM, -1.0, cf_C_cN,
                     Constants::DIM, cf_upper_S_rl, 1, 1.0, cf_sum_torque_spring_car, 1);
        mkl<T>::gemv(CblasRowMajor, CblasTrans, Constants::DIM, Constants::DIM, -1.0, cf_C_cN,
                     Constants::DIM, cf_upper_S_fr, 1, 1.0, cf_sum_torque_spring_car, 1);
        mkl<T>::gemv(CblasRowMajor, CblasTrans, Constants::DIM, Constants::DIM, -1.0, cf_C_cN,
                     Constants::DIM, cf_upper_S_fl, 1, 1.0, cf_sum_torque_spring_car, 1);

        mkl<T>::gemv(CblasRowMajor, CblasTrans, Constants::DIM, Constants::DIM, 1.0, cf_C_cN,
                     Constants::DIM, cf_sum_car_force_fl, 1, 0.0, cf_temp, 1);
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, 1.0,
                     this->r_fl_tilda, Constants::DIM, cf_temp, 1, 1.0, cf_sum_torque_spring_car,
                     1);
        mkl<T>::gemv(CblasRowMajor, CblasTrans, Constants::DIM, Constants::DIM, 1.0, cf_C_cN,
                     Constants::DIM, cf_sum_car_force_fr, 1, 0.0, cf_temp, 1);
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, 1.0,
                     this->r_fr_tilda, Constants::DIM, cf_temp, 1, 1.0, cf_sum_torque_spring_car,
                     1);
        mkl<T>::gemv(CblasRowMajor, CblasTrans, Constants::DIM, Constants::DIM, 1.0, cf_C_cN,
                     Constants::DIM, cf_sum_car_force_rl, 1, 0.0, cf_temp, 1);
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, 1.0,
                     this->r_rl_tilda, Constants::DIM, cf_temp, 1, 1.0, cf_sum_torque_spring_car,
                     1);
        mkl<T>::gemv(CblasRowMajor, CblasTrans, Constants::DIM, Constants::DIM, 1.0, cf_C_cN,
                     Constants::DIM, cf_sum_car_force_rr, 1, 0.0, cf_temp, 1);
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::DIM, Constants::DIM, 1.0,
                     this->r_rr_tilda, Constants::DIM, cf_temp, 1, 1.0, cf_sum_torque_spring_car,
                     1);
    }
    ...
    void construct_right_hand_side()
    {
        ...
        */
        ...
        mkl<T>::axpy(Constants::DIM, 1.0, FW_fl, 1, brem_start, 1);
        ...
        mkl<T>::axpy(Constants::DIM, 1.0, FW_fr, 1, brem_start, 1);
        ...
        mkl<T>::axpy(Constants::DIM, 1.0, FW_rl, 1, brem_start, 1);
        ...
        mkl<T>::axpy(Constants::DIM, 1.0, FW_rr, 1, brem_start, 1);
    ...
    void solve_accelerations()
    {
        ...
        MathLibrary::vector_elem_wise_product<T>(A_rem, cf_b_rem, accelerations,
                                                 9 * Constants::DIM);
    }
    ...
    void compute_quaternion_change_rate()
    {
        ...
        Qc = 0.5 * [qc(4) -qc(3) qc(2); qc(3) qc(4) -qc(1); -qc(2) qc(1) qc(4); -qc(1) -qc(2)
        -qc(3)]; qc_dot = Qc * wc;
        */
        ...
        cf_Qc[10] = -0.5 * qc_[1];
        cf_Qc[11] = -0.5 * qc_[2];
        mkl<T>::gemv(CblasRowMajor, CblasNoTrans, Constants::NUM_LEGS, Constants::DIM, 1.0, cf_Qc,
    ...
    void construct_f_vector(double* f_)
    {
        ...
        mkl<T>::copy(9 * Constants::DIM, accelerations, 1, start_next, 1);
    ...
    void solve(T* solution_vector)
    {
        ...
        */
        ...
        if (used_solver == BROYDEN_CN) {
            MathLibrary::Solvers<T, MBD_method>::Broyden_CN(this, x_vector, complete_vector,
                                                            this->h, this->num_iter, this->tol,
                                                            this->max_iter);
        }
        else if (used_solver == RUNGE_KUTTA_4) {
            MathLibrary::Solvers<T, MBD_method>::RK4(this, x_vector, complete_vector, this->h,
                                                     this->num_iter, this->tol, this->max_iter);
        }
        else if (used_solver == BROYDEN_BDF2) {
            MathLibrary::Solvers<T, MBD_method>::Broyden_PDF2(this, x_vector, complete_vector,
                                                              this->h, this->num_iter, this->tol,
                                                              this->max_iter);
        }
        else if (used_solver == BROYDEN_EULER) {
            MathLibrary::Solvers<T, MBD_method>::Broyden_Euler(this, x_vector, complete_vector,
                                                               this->h, this->num_iter, this->tol,
                                                               this->max_iter);
        }
