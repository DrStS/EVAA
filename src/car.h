...
private:
	...
	*/
	void Construct11DOFMass() {
		M_linear[0] = massComponents[0];
		M_linear[Constants::DOF + 1] = momentOfInertia[0];
		M_linear[2 * Constants::DOF + 2] = momentOfInertia[4];
		mkl<T>::copy(Constants::DOF - 3, massComponents + 1, 1, M_linear + 3 * Constants::DOF + 3, Constants::DOF + 1); // M_linear = diagonal matrix
	}
...
public:
	...
	*/
	...
	T k_body_fl;	// to be TEO 
	T k_tyre_fl;	// to be TEO 
	T k_body_fr;	// to be TEO 
	T k_tyre_fr;	// to be TEO 
	T k_body_rl;	// to be TEO 
	T k_tyre_rl;	// to be TEO 
	T k_body_rr;	// to be TEO 
	T k_tyre_rr;	// to be TEO 
	
	T * M_linear, *temp_linear, *K, *K_trans, *D;   // put into Shubham 2Track-model
	T * k_vec, *l_lat, *l_long;		// to be TEO
	...
	*/
	Car(EVAALookup<Constants::floatEVAA> * interpolator) {
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		lookupStiffness = interpolator;    // to be TEO (MDB) (not MBD)
		...
		*massFullCar = getMassFullCar();
		...
		if (MetaDataBase::DataBase()->getUseInterpolation()) {
			lookupStiffness->getInterpolation(currentSpringsLength, k_vec);
		}
		else {
