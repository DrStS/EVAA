 ...
 */
template <typename T>
void writeMatrixToFile(const std::string& fileName, const std::string& matrixName, T* matrix, size_t numRows, size_t numColumns);
 ...
 */
template <typename T>
void readMatrixFromFile(const std::string& fileName, const std::string& matrixName, T* matrix, size_t numRows, size_t numColumns);
...
class OutputHDF5 {
public:
    OutputHDF5(std::string _fileName, std::string _filePath) : myFileName(_fileName), myFilePath(_filePath) {
#ifdef USE_HDF5
    ...
    */
    ...
    void WriteMatrix(const std::string& _datasetName, const T* _data, const size_t _numRows, const size_t _numColumns, const HDF5DataType& datatype) {
        /*
         ...
         */
        H5::DataSet* dataset = new H5::DataSet(myHDF5FileHandle->createDataSet(_datasetName, datatype, dataspace));
     ...
     *
     */
    void WriteMatrix(const std::string& _datasetName, const T* _data, const size_t _numRows, const size_t _numColumns) {
     ...
     */
    template <typename HDF5DataType>
    void ReadMatrix(const std::string& _datasetName, T* _data, size_t& _numRows, size_t& _numColumns, const HDF5DataType& datatype) {
        // Open dataset
     ...
     *
     */
    void ReadMatrix(const std::string& _datasetName, T* _data, size_t& _numRows, size_t& _numColumns) {
        const getHDF5DataType<T> HDF5DataType;
     ...
     */
    template <typename HDF5DataType>
    void WriteVector(const std::string& _datasetName, const T* _data, const size_t _numElements, const HDF5DataType& datatype) {
        /*
         * Define the size of the array and create the data space for fixed
         * size dataset.
         */
        ...
        // TODO: useless information!!! Run h5dump.exe and see that it is redundant
        H5::DataSpace attrDataspaceScalar(H5S_SCALAR);

        H5::Attribute attribute = dataset->createAttribute("Vector # elements", H5::PredType::STD_I32BE, attrDataspaceScalar);
     ...
     */
    template <typename HDF5DataType>
    void ReadVector(const std::string& _datasetName, T* _data, size_t& _numElements, const HDF5DataType& datatype) {
        /*
         * Create a new dataset within the file using defined dataspace and
         * datatype and default dataset creation properties.
         */
        ...
        // TODO read dimensions directly from hdf5 DATASPACE SIMPLE (remove attributes part) (use h5dump.exe)
