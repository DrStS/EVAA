...
private:
    ...
    void ComputeInternalTorqueFromLagrangianForceFixed(T* Torque, T* F_vec) {
        // calculate Eulerian force out of the Lagrangian
        T angle = _carObj->_currentAngleLagrangian;
        T c = std::cos(angle);
        T s = std::sin(angle);
     ...
     * \param Torque current torque of the body [XYZ]
     */
    void ComputeInternalTorqueFromEulerianForces(T* Torque) {

        // CoG
        Torque[0] -=
            _eulerianForce[0] * _carObj->getCurrentCIRTwoTrackModel()[1];
        Torque[1] -=
            _eulerianForce[0] * _carObj->getCurrentCIRTwoTrackModel()[0];

        // wheels and tyres
        // compute torque around X-axis
        for (auto i = 1; i < 2 * Constants::NUM_LEGS + 1; ++i) {
            Torque[0] -= _eulerianForce[i + 2] *
                         _carObj->getCurrentCIRTwoTrackModel()[i * Constants::DIM + 1];
        }

        // compute torque around Y-axis
        for (auto i = 0; i < 2 * Constants::NUM_LEGS + 1; ++i) {
            Torque[1] -= _eulerianForce[i + 2] *
                         _carObj->getCurrentCIRTwoTrackModel()[i * Constants::DIM + 0];
    ...
    void ReadExternalForce() {
        ...
        position_start =
            externalForce + 2 * Constants::DIM;  // skip 3 for center of mass and 3 for the wheel
        Math::copy<T>(Constants::DIM, _xmlStart, 1, position_start, 1);
        // T2 = T_fr
...
public:
    ...
    LoadModule(Profile<T>* Profile_type, Car<T>* Car1) :
        _activeProfile(Profile_type), _carObj(Car1) {
    ...
    LoadModule(Lagrange<T>* lagrangeProfile, Euler<T>* eulerProfile, Car<T>* Car1) :
        //
        _lagrangianProfile(lagrangeProfile),
        _eulerianProfile(eulerProfile),
        _carObj(Car1) {
        aleCounter = true;
        ...
        if (eulerProfile->GetName() == "Fixed" || eulerProfile->GetName() == "Sinusoidal") {
            ComputeInternalTorqueFromLagrangianForce =
                &LoadModule<T>::ComputeInternalTorqueFromLagrangianForceFixed;
        }
        else if (eulerProfile->GetName() == "Nonfixed") {
            ComputeInternalTorqueFromLagrangianForce =
                &LoadModule<T>::ComputeInternalTorqueFromLagrangianForceNonFixed;
        }
    }
    ~LoadModule() {
    ...
    std::string GetEulerProfileName() { return _eulerianProfile->GetName(); }

    std::string GetLagrangianProfileName() { return _lagrangianProfile->GetName(); }

    void GetLagrangianForce(const size_t& _iterationCount, T* lagrangeForce) {
        _lagrangianProfile->GetProfileForceLagrangian(_iterationCount, _carObj, _lagrangianForce,
                                                      lagrangeForce);
...
#pragma loop(ivdep)
    ...
    void GetEulerianForce(const size_t& _iterationCount, T* eulerForce) {
        // compute reaction force based on current computed _lagrangianForce always called after
        // GetLagrangian Force otherwise gives wrong result
...
#pragma loop(ivdep)
        ...
        for (auto i = 0; i < Constants::NUM_LEGS; ++i) {
            Math::axpy<T>(Constants::DIM, -0.25, reactionForce, Constants::INCX,
                          _lagrangianForce + Constants::TYRE_INDEX_LAGRANGE[i], Constants::INCX);
        }
