...
//#endif
//}
void EVAAComputeEngine::Roadtest(void) {
	Lagrange<Constants::floatEVAA>* roadProfile;
	Car<Constants::floatEVAA>* car = new Car<Constants::floatEVAA>();	
	auto& db = MetaDataBase<Constants::floatEVAA>::getDataBase();

	roadProfile = new Circular<Constants::floatEVAA>(db.getCircularRoadCenter(), db.getCircularRoadRadius());
	Constants::floatEVAA* profileInducedForce, *reactionOnTyre;
	profileInducedForce = Math::malloc<Constants::floatEVAA>((Constants::DIM - 1)*Constants::VEC_DIM);
	reactionOnTyre = Math::malloc<Constants::floatEVAA>((Constants::DIM - 1));
	std::cout << "This is Lagrange Frame" << std::endl;
	roadProfile->ApplyProfileInitialCondition(car);
	std::cout << "Initial condition currentAngularVelocityLagrangian = " << car->currentAngularVelocityLagrangian << std::endl;
	roadProfile->GetProfileForceLagrangian(car, profileInducedForce, reactionOnTyre);
	std::cout << "profile induced force =" << std::endl;
	Math::write_vector(profileInducedForce, 18);
	std::cout << "reactionOnTyre force =" << std::endl;
	Math::write_vector(reactionOnTyre, 2);

	Euler<Constants::floatEVAA>* eulerroadProfile;
	eulerroadProfile = new Fixed<Constants::floatEVAA>(db.getGravityField()[2]);
	Constants::floatEVAA* profileInducedForceeuler;
	profileInducedForceeuler = Math::malloc<Constants::floatEVAA>(Constants::DOF);
	eulerroadProfile->ApplyProfileInitialCondition(car);
	std::cout << "This is Euler Frame" << std::endl;
	std::cout << "Initial condition displacement =" << std::endl;
	Math::write_vector(car->currentDisplacementTwoTrackModel, Constants::DOF);
	std::cout << "Initial condition spring length =" << std::endl;
	Math::write_vector(car->currentSpringsLength, 2 * Constants::NUM_LEGS);
	std::cout << "Initial condition global position =" << std::endl;
	Math::write_vector(car->currentPositionTwoTrackModel, Constants::DOF);
	eulerroadProfile->GetProfileForceEulerian(car, profileInducedForceeuler);
	std::cout << "profile induced force =" << std::endl;
	Math::write_vector(profileInducedForceeuler, Constants::DOF);
