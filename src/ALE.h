...
public:
    ...
    ALE(Car<T>* carObjVal, LoadModule<T>* loadModuleVal, TwoTrackModelParent<T>* twoTrackModelVal,
        std::string filePath = "", std::string fileName = ".hdf5") :
        //
        _carObj(carObjVal),
        ...
        _fullSolution =
            Math::malloc<T>(_solutionVectorSize * (Constants::VEC_DIM * Constants::DIM));

        // initialize solution vector
        _timeArray = Math::calloc<T>(_solutionVectorSize);  // TODO Allocation in constructor
        _lagrangianForceVector =
            Math::calloc<T>(Constants::lagrangianForceDimension);  // TODO Allocation in constructor
        ...
        velVecCSV =
            Math::malloc<T>(_solutionVectorSize * (Constants::DIM - 1) * Constants::VEC_DIM);
#endif  // WRITECSV
    ...
    void LagrangianUpdate(const size_t iter) {
        ...
        Math::Solvers<T, ALE>::StoermerVerletPosition(
            _carObj->_currentPositionLagrangian[0], _carObj->getCurrentVelocityLagrangian()[0],
            _lagrangianForceVector[0], _h, _carObj->getMassCarFull());
        Math::Solvers<T, ALE>::StoermerVerletPosition(
            _carObj->_currentPositionLagrangian[1], _carObj->getCurrentVelocityLagrangian()[1],
            _lagrangianForceVector[1], _h, _carObj->getMassCarFull());

        // 2. Update Z-rotation
        Math::Solvers<T, ALE>::StoermerVerletPosition(
            _carObj->_currentAngleLagrangian, _carObj->getCurrentAngularVelocityLagrangian(),
            _lagrangianTorque, _h, _momentOfInertiaZ);
        ...
        Math::Solvers<T, ALE>::StoermerVerletVelocity(
            _carObj->_currentVelocityLagrangian[0], _lagrangianForceVector[0],
            _newLagrangianForceVector[0], _h, _carObj->getMassCarFull());
        Math::Solvers<T, ALE>::StoermerVerletVelocity(
            _carObj->_currentVelocityLagrangian[1], _lagrangianForceVector[1],
            _newLagrangianForceVector[1], _h, _carObj->getMassCarFull());

        // 5. Update Z-angular velocities
        Math::Solvers<T, ALE>::StoermerVerletVelocity(_carObj->_currentAngularVelocityLagrangian,
                                                      _lagrangianTorque, _newLagrangianTorque, _h,
                                                      _momentOfInertiaZ);
    ...
    void Solve(T* finalSolution) {
        ...
        for (iter = 1, t = _h; std::abs(t - (_tend + _h)) > eps; iter++, t += _h) {
            ...
            Math::copy(Constants::VEC_DIM * Constants::DIM, _carObj->getPositionVector(), 1,
                       posVecCSV + iter * Constants::DIM * Constants::VEC_DIM, 1);
            Math::copy(Constants::DIM, _carObj->getAngleCG(), 1,
                       angleVecCSV + iter * Constants::DIM, 1);
            Math::copy(Constants::DIM, _carObj->_currentVelocityLagrangian, 1,
                       velVecCSV + iter * (Constants::DIM - 1) * Constants::VEC_DIM, 1);
#endif  // WRITECSV
        ...
        Math::copy<T>(Constants::VEC_DIM * Constants::DIM,
                      _fullSolution + (iter - 1) * (Constants::VEC_DIM * Constants::DIM), 1,
                      finalSolution, 1);
        _carObj->CombineResults();
    }
    ...
    void CalculateGlobalMomentofInertiaZ() {
        ...
        _momentOfInertiaZ += (_carObj->getMassComponents()[1] + _carObj->getMassComponents()[2]) *
                             (_carObj->_lenLat[0] * _carObj->_lenLat[0] +
                              _carObj->_lenLong[0] * _carObj->_lenLong[0]);
        _momentOfInertiaZ += (_carObj->getMassComponents()[3] + _carObj->getMassComponents()[4]) *
                             (_carObj->_lenLat[1] * _carObj->_lenLat[1] +
                              _carObj->_lenLong[1] * _carObj->_lenLong[1]);
        _momentOfInertiaZ += (_carObj->getMassComponents()[5] + _carObj->getMassComponents()[6]) *
                             (_carObj->_lenLat[2] * _carObj->_lenLat[2] +
                              _carObj->_lenLong[2] * _carObj->_lenLong[2]);
        _momentOfInertiaZ += (_carObj->getMassComponents()[7] + _carObj->getMassComponents()[8]) *
                             (_carObj->_lenLat[3] * _carObj->_lenLat[3] +
                              _carObj->_lenLong[3] * _carObj->_lenLong[3]);
    }
    ...
    void WriteBulkResults(std::string filePath = "",
                          std::string fileName = "ALE_full_solution.hdf5",
        ...
        fullALE.WriteMatrix(datasetName, _fullSolution, _solutionVectorSize,
