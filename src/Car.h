...
public:
    ...
    */
    ...
    Car() {
        ...
        kVec = Math::calloc<T>(2 * Constants::NUM_LEGS);
        ...
        // CG
        Math::copy<T>(Constants::DIM, db.getBodyInitialPosition(), 1, initialPositionGlobal, 1);  // copy the center of mass position
     ...
     */
    void combineEulerianLagrangianVectors(T* global_vector) {
        ConvertALEToGlobal(currentPositionLagrangian, global_vector);
        Convert11DOFToGlobal(currentPositionTwoTrackModel, global_vector);
    }
    ...
    void combine_results() {
		...
		w_CG[0] = currentVelocityTwoTrackModel[1];
		w_CG[1] = currentVelocityTwoTrackModel[2];
        w_CG[2] = currentAngularVelocityLagrangian;
        ConvertALEToGlobal(currentVelocityLagrangian, Velocity_vec);
		Convert11DOFToGlobal(currentVelocityTwoTrackModel, Velocity_vec);
    }
    ...
    void get_Velocity_vec_xy(T* Vel) {
        Math::copy<T>((Constants::DIM - 1) * Constants::VEC_DIM, currentVelocityLagrangian, 1, Vel, 1);
    }
