...
// Until MKL does not get an alternative for this project's purpose, keep the methods in the
// EVAA::Math namespace to write cleaner code.
// namespace MKL {
 ...
 * - make use of std::forward to stop writing all arguments. The downside is that you might not get
 *   autocomplete features.
 * - Copy EIGEN's approach: declare all [sb] versions of the functions using the same base name
 *   since C++ allows overloads, e.g. define apxy(float) and apxy(double) letting the type to be
 *   inferred when used.
 * */
namespace {
...
/**
 * \brief Allocates a buffer memory for count elements and initializes them to 0.
 * Wraps a call to mkl_calloc.
 * \param count Number of elments.
 * \return The newly allocated buffer.
 */
template <typename T>
...
/**
 * \brief Frees buffer memory previously allocated with Math::malloc or Math::calloc.
 * Wraps a call to mkl_free.
...
TEMPLATE_FLOAT_TYPE
inline lapack_int ggsvd3(int matrix_layout, char jobu, char jobv, char jobq, lapack_int m,
...
TEMPLATE_FLOAT_TYPE
inline lapack_int tgsja(int matrix_layout, char jobu, char jobv, char jobq, lapack_int m,
...
TEMPLATE_FLOAT_TYPE
inline int dfInterpolateEx1D(DFTaskPtr task, const MKL_INT type, const MKL_INT method,
                             const MKL_INT nsite, const T* site, const MKL_INT sitehint,
                             const MKL_INT ndorder, const MKL_INT* dorder, const T* datahint, T* r,
    ...
    return invokeFD(dfsInterpolateEx1D, dfdInterpolateEx1D, task, type, method, nsite, site,
                    ...
                    re_params, NULL, i_params, NULL, search_params);
}
