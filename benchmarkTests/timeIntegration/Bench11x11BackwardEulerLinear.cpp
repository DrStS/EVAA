...
static void BM_MKL11x11BackwardEulerLinear(benchmark::State& state) {
	for (auto _ : state) {
		...
		mkl_jit_create_dgemm(&jitter, MKL_COL_MAJOR, MKL_NOTRANS, MKL_NOTRANS, DOF, 1, DOF, 1.0, DOF, DOF, 0.0, DOF);
...
BENCHMARK(BM_MKL11x11BackwardEulerLinear)->UseRealTime()->Arg(10)->Arg(15)->Arg(20);
#endif

#ifdef USE_EIGEN
static void BM_Eigen11x11BackwardEulerLinear(benchmark::State& state) {
	for (auto _ : state) {
		int DOF = 11;
...
BENCHMARK(BM_Eigen11x11BackwardEulerLinear)->UseRealTime()->Arg(10)->Arg(15)->Arg(20);
#endif

#ifdef USE_BLAZE
static void BM_Blaze11x11BackwardEulerLinear(benchmark::State& state) {
	for (auto _ : state) {
...
BENCHMARK(BM_Blaze11x11BackwardEulerLinear)->UseRealTime()->Arg(10)->Arg(15)->Arg(20);
#endif
